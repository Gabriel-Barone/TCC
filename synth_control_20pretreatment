...
for(r in 1:(nrow(diagnostic)/4)) {
  ...
  tryCatch({
    V_out_predictors <-
      ...
      generate_predictor(time_window = 30:50,
                         # V = mean(V, na.rm = T),
                         H1 = mean(`H1`, na.rm = T),
                         H2 = mean(`H2`, na.rm = T),
                         H3 = mean(`H3`, na.rm = T),
      ...
      generate_weights(optimization_window = 30:50, # time to use in the optimization task
      ) %>%
    ...
    synth_control <- synth_control[synth_control$time_unit >= 30 & synth_control$time_unit <= 50,]
  ...
  if (r %% 50 == 0) {
    ...
    filename <- paste0("tcc_diagnostic_20pretreatment_1_to_", (iteration_count * 50), ".csv")
